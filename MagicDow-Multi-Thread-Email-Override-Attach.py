...
#csv_file = os.path.join(OUTPUT_DIR, f"Dow30_magic_formula_{today}.csv")
csv_file = f"Dow30_magic_formula_{today}.csv"
#excel_file = os.path.join(OUTPUT_DIR, f"Dow30_magic_formula_{today}.xlsx")
excel_file = f"Dow30_magic_formula_{today}.xlsx"
...
if fail_percent <= FAIL_THRESHOLD:
    subject = f"Dow30 Magic Formula Results ({today}) - Success"
    plain_body = f"Attached are the ranking files and error log. Failure rate: {fail_percent:.2f}%."
    html_body = f"""
    <html><body>
    <h2>Dow30 Magic Formula Results ({today})</h2>
    ...
    subject = f"Dow30 Magic Formula Run ({today}) - High Failure Rate Alert (with partial summary)"
    if not df.empty:
        top5 = df.head(5)
